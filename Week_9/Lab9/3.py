...
pygame.draw.polygon(screen,color_set['black'],([360,HEIGHT-40],[360,HEIGHT],[400,HEIGHT]),3) # range: 360 - 400
...
while run:
   ...
   for event in pygame.event.get():
      ...
      if draw == 'right triangle':
         if event.type == pygame.MOUSEBUTTONDOWN:
            prev = pos_select
         if event.type == pygame.MOUSEBUTTONUP:
            cur = pygame.mouse.get_pos()
            if prev[1] < HEIGHT-50 and cur[1] < HEIGHT-50:
               x_start,y_start = prev[0], prev[1]
               x_center, y_center = prev[0], cur[1]
               x_end, y_end = cur[0], cur[1]
               pygame.draw.polygon(screen,color,([x_start,y_start], [x_center,y_center], [x_end,y_end]),line_width)
